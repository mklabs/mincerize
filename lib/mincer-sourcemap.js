...
// Stylus renderer
MinmapStream.prototype.stylus = function _stylus(filename, body, done) {
  var self = this;
  stylus(body, { firebug: true })
    .set('filename', filename)
    .render(function(err, css) {
      if(err && err.name !== 'ParseError') return done(err);
      if(err) {
        console.error('... Warning: Will not generate debug info for %s. Stylus %s ...', filename, err.name);
      ...
      var pattern = /^@media -[\w]+-debug-info\{filename\{font-family:([^\}]+)\}line\{font-family:\\00003([\d]+)\}\}/;
      var lines = css.split('\n').map(function(l) {
        if(!pattern.test(l)) return l;
        var parts = l.match(pattern);
        var line = parts[2];
        return self.debugInfo(filename, line);
      });
...
MinmapStream.prototype.rework = function _rework(filename, body, done) {
  ...
  lines.forEach(function(line, i) {
    ...
    var info = infos.filter(function(info) {
      ...
      info.selector = info.selector.replace(/\*/, '\\*');
      return new RegExp('^\\s*' + info.selector).test(line);
    })[0];
