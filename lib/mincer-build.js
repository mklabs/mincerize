...
MincerBuild.prototype.configure = function configure() {
  ...
  if (this.options.compress) this.environment.jsCompressor  = "uglify";
  // (!) use csswring, because csso does not supports sourcemaps
  if (this.options.compress) this.environment.cssCompressor = "csswring";
...
MincerBuild.prototype.compile = function compile(done) {
  ...
  this.log('Assets:', this.assets);
  var self = this;

  var options = {};
  if (this.options.sourcemap) options.sourceMaps = true;
  if (this.options.sourcemap) options.embedMappingComments = true;

  this.manifest.compile(this.normalize(this.assets), options);
...
MincerBuild.prototype.replace = function replace(done) {
  ...
  assets.forEach(function(asset) {
    body = body.replace(asset, function() {
      return compiled[self.normalize(asset)];
    });
  });
